...
const eventConfig = {
  deposit_submitted:       { icon: '↓', color: 'text-blue-500',    bg: 'bg-blue-500/10' },
  deposit_credited:       { icon: '✓', color: 'text-green-500',   bg: 'bg-green-500/10' },
  deposit_rejected:       { icon: '✕', color: 'text-red-500',     bg: 'bg-red-500/10' },
  sell_order_created:     { icon: '↑', color: 'text-blue-500',    bg: 'bg-blue-500/10' },
  ...
  page.innerHTML = `
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-muted mt-1">Your account activity</p>
      </div>
    ...
    if (notifications.length === 0) {
      container.innerHTML = `
        <div class="card flex flex-col items-center py-16 text-center">
          ...
          <p class="mt-1 text-[13px] text-text-secondary dark:text-text-secondary-dark">Your account activity will appear here</p>
        </div>
      `;
  ...
  function createNotifCard(n) {
    ...
    card.innerHTML = `
      <div class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full ${style.bg}">
        <span class="text-[16px] font-bold ${style.color}">${style.icon}</span>
      </div>
